...
// SampleGoroutineWorkflow This sample workflow demonstrates how to use multiple Temporal goroutines (instead of native goroutine)
// to process a sequence of activities parallel.
...
func SampleGoroutineWorkflow(ctx workflow.Context, parallelism int) (results []string, err error) {
	...
	for i := 0; i < parallelism; i++ {
		...
		workflow.Go(ctx, func(gCtx workflow.Context) {
			...
			var result1 string
			future1 := workflow.ExecuteActivity(gCtx, Step1, input1)
			err = future1.Get(gCtx, &result1)
			if err != nil {
				// Very native error handling. Only the last error will be returned by the workflow.
				return
			}

			var result2 string
			future2 := workflow.ExecuteActivity(gCtx, Step2, result1)
			err = future2.Get(gCtx, &result2)
			if err != nil {
				return
			}
