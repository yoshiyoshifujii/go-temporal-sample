...
/**
 * Dummy server that support to list expenses, create new expense, update expense and checking expense state.
 */
...
func listHandler(w http.ResponseWriter, _ *http.Request) {
	...
	for _, id := range keys {
		...
		_, _ = fmt.Fprintf(w, "<tr><td>%s</td><td>%s</td><td>%s</td>", id, state, actionLink)
	}
...
func notifyExpenseStateChange(id, state string) {
	...
	if err != nil {
		...
		fmt.Printf("Successfuly complete activity: %s\n", token)
	}
}
